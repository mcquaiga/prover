...
using Prover.InstrumentProtocol.Core;
...
namespace Prover.InstrumentProtocol.Honeywell.Domain.Instrument
{
    internal class HoneywellInstrument : IInstrument, IDisposable
    {
        ...
        internal HoneywellInstrument(int id, int accessCode, string name, string itemFilePath,
        ...
        internal HoneywellInstrument(HoneywellClient commClient, int id, int accessCode, string name,
            string itemFilePath)
            : this(id, accessCode, name, itemFilePath)
        {
        ...
        public EvcCommunicationClient CommunicationClient => CommClient;
        ...
        public virtual string TypeIdentifier => GetType().AssemblyQualifiedName;
        ...
        {
            var items = ItemDefinitions.ToList();
            ItemValues = await DownloadItemValues(items);
        }
        ...
        protected async Task<IEnumerable<ItemValue>> DownloadItemValues(IEnumerable<ItemMetadata> items, bool disconnectAfter = false)
        {
            var itemMetadatas = items?.ToList();

            if (IsReadOnly || itemMetadatas == null || !itemMetadatas.Any()) return new List<ItemValue>();

            await CommClient.Connect(this, CancellationToken.None);
            var itemValues = await CommClient.GetItemValues(itemMetadatas);

            if (disconnectAfter) await CommClient.Disconnect();
        ...
        protected IEnumerable<ItemValue> SetupItemValues(IEnumerable<ItemMetadata> itemsMetadata,
            Dictionary<string, string> itemData)
        {
